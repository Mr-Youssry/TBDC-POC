...
export interface UpdateInvestorDeps {
  assistantUserId: string;
}

export function registerUpdateInvestor(
  api: RegisterApi,
  prisma: PrismaClient,
  deps: UpdateInvestorDeps,
) {
  api.registerTool({
    name: "update_investor",
    label: "Update Investor",
    description:
      "Update editable fields on an Investor row. Does NOT touch id/name/sortOrder. Requires actingUserId. Wraps write + audit insert in a transaction.",
    parameters: {
      type: "object",
      properties: {
