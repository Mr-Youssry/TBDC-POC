...
export function registerUpdateMatch(
  api: RegisterApi,
  prisma: PrismaClient,
  ...
  api.registerTool({
    ...
    label: "Update Match",
    description:
      "Update a Match row's editable fields (tier, score, rationale, nextStep, warmPath, portfolioGap). Requires actingUserId — the admin User.id whose session authorized the edit. Wraps the write + audit insert in a single transaction; if either fails, neither lands.",
    parameters: {
      type: "object",
      properties: {
        matchId: {
        ...
        actingUserId: {
          type: "string",
          description:
            "User.id of the admin on whose behalf the assistant is making the edit. Injected by the chat proxy from the NextAuth session — never from the LLM directly.",
        },
        patch: {
          type: "object",
        ...
        chatSessionId: {
          type: "string",
          description:
            "Optional OpenClaw chat session id for audit traceability.",
        },
      },
