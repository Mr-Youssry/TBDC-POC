...
export interface AppendAuditNoteDeps {
 ...
 */
export function registerAppendAuditNote(
  api: RegisterApi,
  prisma: PrismaClient,
  deps: AppendAuditNoteDeps,
) {
  api.registerTool({
    name: "append_audit_note",
    label: "Append Audit Note",
    description:
      "Append a free-text note to the AuditLog without mutating any content row. Use this to record rationale-only observations (e.g. why you did NOT create a match). Requires actingUserId.",
    parameters: {
      type: "object",
      properties: {
        actingUserId: {
          type: "string",
          description:
            "User.id of the admin on whose behalf the note is being recorded.",
        },
        tableName: {
          type: "string",
          description:
            "Table the note is about (e.g. 'Match', 'Company'). Free-text — used for filtering in /admin/audit.",
        },
        rowId: {
          type: "string",
          description:
            "Row id the note is about. Use 'n/a' if the note is not about a specific row.",
        },
        note: {
          type: "string",
