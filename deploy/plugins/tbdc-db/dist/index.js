...
export default definePluginEntry({
    id: "tbdc-db",
    name: "TBDC DB",
    description: "Read/write access to the TBDC Investor Matchmaking Postgres DB. Every write records an AuditLog row attributed to the assistant user, on behalf of the authenticated admin (actingUserId).",
    configSchema: {
        type: "object",
        properties: {
    ...
    async register(api) {
        ...
        const databaseUrl = (typeof config.databaseUrl === "string" && config.databaseUrl) ||
            process.env.TBDC_DATABASE_URL ||
            process.env.DATABASE_URL ||
            "";
        if (!databaseUrl) {
            api.logger?.error?.("[tbdc-db] no databaseUrl in config and no TBDC_DATABASE_URL/DATABASE_URL in env — refusing to register tools");
            return;
        }
        const assistantEmail = (typeof config.assistantUserEmail === "string" &&
      ...
      `);
            if (rows.length > 0 && rows[0]) {
                ...
                api.logger?.info?.(`[tbdc-db] assistant user resolved: ${assistantEmail} → ${rows[0].id}`);
            }
            else {
                api.logger?.warn?.(`[tbdc-db] assistant user not found for email ${assistantEmail}; write tools will be skipped`);
            }
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
            api.logger?.error?.(`[tbdc-db] failed to resolve assistant user: ${msg}; write tools will be skipped`);
        }
        ...
        if (assistantUserId) {
            ...
            api.logger?.info?.("[tbdc-db] registered 4 read tools (write tools disabled — no assistant user)");
        }
    },
});
